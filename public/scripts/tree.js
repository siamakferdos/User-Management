var Tree = function (name, data, target) {       
        ...
        this.mapDataSource = {id : "id", name: "name", parentId: "parentId", extraData: "extraData"};
        var rootId = null;
        var multiSelect = false;
        var allowEdit = false;
        var allowMoveItem = false;
        var mapDataSource = {id : "id", name: "name", parentId: "parentId", extraData: "extraData"} ;
        var treeData = data;
         ...
         if(this.mapDataSource) 
            mapDataSource = this.mapDataSource;
             ...
             treeData.sort(function(a, b){
                 return b[mapDataSource.id] - a[mapDataSource.id];
             })
         
            Array.prototype.push.apply(mainStack, treeData.filter(function(value){
                    return (value[mapDataSource.parentId] == rootId)
                })
            ); 
            ...
            indexStack.push({id: mainStack[mainStack.length - 1][mapDataSource.id], index: mainStack.length - 1});
            ...
            while (indexStack.length > 0) { 
                ...
                treeHtml += "<li bb data-value='"+ mainStack[mainStack.length - 1][mapDataSource.id]+"' " +
                "data-extra='" + mainStack[mainStack.length - 1][mapDataSource.extraData] + "' >"
                if(tempStack.length > 0)
                    treeHtml += "<img data-status src='" + rtlStatusImage + "'>";
                else    
                    treeHtml += "<span data-space></span>";
                treeHtml += addCheckBox(mainStack[mainStack.length - 1][mapDataSource.id], this.allowCheck)
                ...
                treeHtml += mainStack[mainStack.length - 1][mapDataSource.name] ;
                treeHtml += "</span>"
                    ...
                    while(tempStack.length > 0){
                        ...
                        treeHtml += "<ul><li aa data-value='"+ mainStack[mainStack.length - 1][mapDataSource.id]+"' " + 
                        "data-extra='" + mainStack[mainStack.length - 1][mapDataSource.extraData] + "' >";
                        if(tempStack.length > 0)
                            treeHtml += "<img data-status src='" + rtlStatusImage + "'>";
                        else    
                    ...
                    while(indexStack.length > 0 && mainStack[mainStack.length - 1][mapDataSource.id] == indexStack[indexStack.length - 1].id){
                        indexStack.pop();
                        mainStack.pop();                
                        treeHtml += "</li></ul>" ; 
                    }  
            ...
            $(function(){
                ...
                $(document).off("click", "div[" + treeNameAttr +"] li img[data-status]").on("click", "div[" + treeNameAttr +"] li img[data-status]", function(event){
                ...
                $(document).off("click", "div[" + treeNameAttr +"] li span[data-text]").on("click", "div[" + treeNameAttr +"] li span[data-text]", function(){
                    var parId = null;
                    var parLi = $(this).closest("ul").closest("li");
                    if(parLi)
                        parId = parLi.attr("data-value")
                    if($(this).hasClass("active")){                        
                        $(this).removeClass("active");
                        if(triggers["unSelect"] != null)
                            triggers["unSelect"]($(this).closest("li").attr("data-value"), $(this).closest("li").attr("data-extra"), parId);
                    }
                    else{
                        if(!multiSelect)
                        ...
                        if(triggers["select"] != null)
                            triggers["select"]($(this).closest("li").attr("data-value"), $(this).closest("li").attr("data-extra"), parId);
                    }
                    
                    
                }) 
                
                $(document).off("dblclick", "div[" + treeNameAttr +"] li span[data-text]").on("dblclick", "div[" + treeNameAttr +"] li span[data-text]", function(){           
                    if(allowEdit){
                        $(this).removeClass("active");
                ...
                $(document).off("click", "div[" + treeNameAttr +"] li button[data-name='btnConfirm']").on("click", "div[" + treeNameAttr +"] li button[data-name='btnConfirm']", function(e){
                ...
                $(document).off("click", "li input[type='text']").on("click", "li input[type='text']", function(e){
                    e.stopImmediatePropagation();   
                    $(this).select().focus();
                })
                ...
                if(allowMoveItem){
                    ...
                    $(document).off("mousedown", "div[" + treeNameAttr +"] li span[data-text]").on("mousedown", "div[" + treeNameAttr +"] li span[data-text]", function(e){
                        $(this).attr("draggable", "true");  
                        drag = drop = null;                 
                    })
                    
                    $(document).off("mouseup", "div[" + treeNameAttr +"] li span[data-text]").on("mouseup", "div[" + treeNameAttr +"] li span[data-text]", function(e){
                        $(this).attr("draggable", "false");
                        // drag = drop = null;                   
                    })
                    
                    $(document).off("dragover", "div[" + treeNameAttr +"] li span[data-text]").on("dragover", "div[" + treeNameAttr +"] li span[data-text]", function(e){
                        e.preventDefault();
                    });
                    
                    $(document).off("dragstart", "div[" + treeNameAttr +"] li span[data-text]").on("dragstart", "div[" + treeNameAttr +"] li span[data-text]", function(e){
                        if(drag == null){
                            drag = $(this).parent("li");
    ...
    function getLastChild(){ 
        ...
        var arr = treeData.filter(function(value){
            return (value[mapDataSource.parentId] == mainStack[mainStack.length - 1][mapDataSource.id]);
        });
        for(var i = 0; i < mainStack.length; i++){
                var currecntIndex = treeData.indexOf(mainStack[i]);
                if(currecntIndex > 0)
                    treeData.splice(currecntIndex, 1);
            }
        return arr;	
    }
